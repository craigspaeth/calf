...
const model = (name, { schema }) => {
  ...
  const wrap = async (fn, ...args) => {
    debug('Retrieving', ...args)
    try {
      const res = await col[fn](...args)
      debug('Received', res)
      return res
    } catch (e) {
      debug('Error', e)
  ...
  const Model = {
    ...
    resolve: (root, opts) => wrap('findOne', typecast(opts))
  }
  const ModelSave = {
    type: ModelType,
    args: schemaToArgs(schema),
    resolve: (root, opts) => wrap('save', validate(opts))
  }
  const Models = {
    type: new GraphQLList(ModelType),
    args: schemaToArgs(schema),
    resolve: (root, opts) => wrap('find', typecast(opts)).sort({ $natural: -1 })
  }
  const ModelDelete = {
    type: ModelType,
    args: schemaToArgs(schema),
    resolve: (root, opts) => wrap('remove', typecast(opts))
  }
