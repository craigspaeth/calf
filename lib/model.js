...
const model = (name, { schema }) => {
  ...
  const Model = {
    ...
    resolve: async (root, opts) => {
      const res = await col.findOne(opts)
      console.log('FIIIND', res)
      return res
    }
  }
  const ModelSave = {
    ...
    resolve: async (root, opts) => {
      const res = await col.save(validate(opts))
      console.log('FIIIND', res.frames[0].firstSection.blocks)
      return res
    }
  }
